...
- **GS1 097.039: NOT_INTENDED_FOR_EU_MARKET**: GS1 rejects this status for MDR/IVDR devices, but these are non-EU market devices (CH, UK, etc.) — EUDAMED only tracks EU/EEA markets, so NOT_INTENDED doesn't specify which non-EU market. Need GS1 rule relaxation for non-EU use cases.
