...
- **download.sh**: Unified download + convert script. Usage: `./download.sh --N` or `./download.sh --srn <SRN> [SRN2 ...] [--N]`. Downloads listing (with optional server-side SRN filtering via API `srn=` parameter; supports multiple SRNs combined into one output, named `<first-SRN>_+<N>srns`), extracts UUIDs, fetches details in parallel (10 concurrent, with retry and resume), downloads Basic UDI-DI data for MDR mandatory fields (cached in `/tmp/basic_udi_cache/`), converts to firstbase JSON.
