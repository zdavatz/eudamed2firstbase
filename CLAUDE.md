...
- TradeItemSynchronisationDates: lastChangeDateTime and effectiveDateTime use EUDAMED version_date; publicationDateTime uses current time; discontinuedDateTime set to today+1 day when status is NO_LONGER_ON_THE_MARKET
...
- **push_to_api.sh**: Automates the workflow with regulatory act routing. Classifies files by `RegulatoryAct` in JSON: MDD/AIMDD/IVDD → `Draft/CreateOne` (draft only, no publish); MDR/IVDR → `Live/CreateMany` (batches of 100) + `AddMany` (publish). Auto-throttles: 1s delay for ≤60 files, 8s for larger batches. Retries HTTP 429. Query status with `--status <reqid>`.
