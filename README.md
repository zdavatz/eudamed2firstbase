...
- **NO_LONGER packaging hierarchy: discontinuedDateTime shape + child-GTIN reject masking** (since v1.0.99/v1.0.100, [#55](https://github.com/zdavatz/eudamed2firstbase/issues/55)): a base unit that is both **NO_LONGER** *and* a **child** in a packaging hierarchy hits a genuine GS1 rule contradiction — `discontinuedDateTime` on every level is rejected by **910.004** (a child cannot be discontinued) + **910.003** (higher level must not post-date the lower), but moving it to the **outermost/despatch level only** (v1.0.99, `transform_detail.rs`) then trips **097.123** ("for discontinued items discontinuedDateTime SHALL be used") on the base-unit child. The correct shape is open with GS1 (the exact failed `CreateMany` payload was sent to GS1 on 06.08.2026 for a TC ticket). Separately, v1.0.100 fixes the **fourth CreateMany masking bug**: a per-item `AttributeException` on a **child** GTIN (e.g. the 097.123 on the base unit while the document's top GTIN is the CASE package) was silently counted ACCEPTED, moved to `processed/`, and AddMany-published — because the accept/reject check keyed only on the document's top-level GTIN. New `doc_all_gtins()` collects every GTIN in a document (top + all nested `CatalogueItemChildItemLink` levels); a document is now rejected if **any** of its GTINs is in `rejected_gtins`. (v1.0.100 also lets `mailto` send a body-only mail with no attachment.)
...
- **Rate-limit-resilient download for all modes + download status bar** (since v1.0.71): the shared `download.rs` path (Mode 0 / `download` CLI / `check`) had the same throttling flaw — at 50 threads EUDAMED's listing endpoint returned **49×429** and the listing loop **aborted pagination on the first 429**, truncating big SRNs (Maik's 82-SRN Mode-0 run pulled only **695 of ~21,813** devices; DE-MF-000018836 got 40 of 4795; 28 SRNs got 0). Now a shared `eudamed_get()` helper **honors `Retry-After` and retries the page** instead of breaking, so every SRN paginates to completion; concurrency is paced to the endpoints' real limits (listing 6 / detail 50 / basic 12). Re-run on the same 82 SRNs: **21,813 UUIDs extracted, 0 truncations**. Plus a **live download progress bar** in the GUI (Listings / detail / basic _done_/_total_) so long rate-limited downloads stay transparent. (Heavily-throttled bulk downloads are now *complete* but inherently slow — bounded by EUDAMED's ~60 req/60 s; a proactive rate-limiter for higher throughput is a planned v1.0.72 follow-up.)
- **Proactive shared rate-limiter — steady max-throughput download** (since v1.0.72): v1.0.71's *reactive* retry-on-429 was correct but **crawled at ~12/min** under heavy throttle (threads each sleeping a 60 s `Retry-After` window in lockstep). Measured: EUDAMED's **~60-req/60 s budget is shared across all device endpoints, per IP** (draining detail's 60 then hitting basic → instant 429). v1.0.72 adds a global `RateLimiter` that **proactively paces every request** (~1 req/1.05 s ≈ 57/min, configurable) just under the shared budget, so 429s never happen — measured **steady 57.4/min, 0 throttles** across 2+ windows mixing detail+basic (≈**4.8× faster** than reactive). It's the **maximum a single IP can do**: ~21,800 devices ≈ ~13 h (per-IP, so separate machines can split the SRN list to parallelize). Thread counts are now throughput-irrelevant; the limiter governs the rate.
- **CLI push to Production** (since v1.0.73): the `repush-srn` subcommand was hardwired to the Test endpoint. It now reads **`FIRSTBASE_ENV=Production`** (anything else, including unset, stays on Test) and pushes to `https://webapi-firstbase.gs1.ch`; the NO_LONGER + already-ACCEPTED skip ([#10](https://github.com/zdavatz/eudamed2firstbase/issues/10)) is keyed to the chosen environment so Test and Production are tracked separately. The push stays SRN-scoped (only the targeted UUIDs). Supplier/recipient GLNs come from `config.toml` `[provider]` defaults (`7612345000480` → token + `DataSource`; `7612345000527` → `PublishToGln`), overridable via `FIRSTBASE_PUBLISH_GLN`. Enables unattended bulk prod pushes (e.g. a 97-SRN customer worklist) without the GUI.
- **Download by GTIN** (since v1.0.74): `download --gtin <GTIN…>` / `--gtin-file <file>` fetches specific devices directly by their UDI-DI primary code, bypassing SRN enumeration. EUDAMED's listing endpoint filters on `primaryDi=<GTIN>` (the *only* parameter that filters — `udiDi`/`code`/`deviceIdentifier`/… are silently ignored and return the full 2.6M-device set), yielding the device's `uuid` (plus its real `manufacturerSrn`, written to `listing_cache` so it can still be pushed by SRN later); the same detail + Basic-UDI fetch, version indexing, and convert then run unchanged. A GTIN with no EUDAMED device is logged (`no device in EUDAMED`) and skipped. Same shared `RateLimiter` (one `primaryDi` request per GTIN, then detail+basic). Also fixed: the `download --convert` step now runs from `app_data_dir()` so the input detail dir, output `firstbase_json/`, DB, and Basic-UDI cache all resolve where `download` actually wrote them (previously it read a cwd-relative `eudamed_json/` that only existed if you ran from the data dir).
...
- **Full ISO 3166-1 country list** (since v1.0.49, [#35](https://github.com/zdavatz/eudamed2firstbase/issues/35)): `country_alpha2_to_numeric` now covers all 250 ISO 3166-1 country codes from the GS1 UDI Connector Profile Apr 2026 V1.1 sheet, up from 48. The previous 48-entry table had a passthrough fallback that emitted the alpha-2 string itself (e.g. `"HK"` instead of `"344"`) which GS1 rejected with G541 on contact addresses. Special cases: `XI` (Northern Ireland) maps to literal `"XI"` per GS1's post-Brexit convention (not numeric `826`); `GB` is aliased to `826` because EUDAMED uses ISO `GB` while the sheet uses non-standard `UK`. `GB` remains filtered from market sales conditions via `is_valid_gdsn_market_country`; **`XI` is emitted since v1.0.106** — the GS1 UDI Connector Profile Apr 2026 V1.1 (`UDID_CodeLists` row 9110) lists `XI` as a valid `salesConditionTargetMarketCountry/countryCode`, so the post-Brexit skip (G541 in early 2026) had become stale and silently dropped Northern Ireland from every device's market list (Maik, 24.09.2026, GTIN 04049154166558; a GS1-side migration failed on the missing market). Proven on GS1 TEST (1/1 accepted with `XI`); 21'289 Production devices (44 SRNs) carried XI in EUDAMED and were re-pushed on 24.09.2026 (21'230 accepted, 10 G485 on NO_LONGER devices whose `discontinuedDateTime` is protected, #10). **v1.0.107:** the push-time `lastChangeDateTime` re-stamp now covers every packaging level — a re-push without `--reconvert` had G488-rejected packaged base units because only the top-level TradeItem was re-stamped. **v1.0.108 — rolling `deep-scan`:** the nightly `check` only sees UDI-DI version bumps (the EUDAMED listing exposes no other version number: `basicUdiDataVersionNumber` is always 0, `lastUpdateDate` always null), so changes to only the Basic UDI-DI, market info, manufacturer/AR, certificates, packages or product designer were never pushed (HumanOptics DE-MF-000017892, 24.09.2026: 2 of 9 migration updates missed). `deep-scan` re-fetches detail + Basic UDI-DI for one of 7 stable slices of the worklist per night (~5'700 devices, ~3.5 h at EUDAMED's 57 req/min), compares every version part and pushes the changed devices — worst-case latency 7 nights instead of never. Also: a same-regulation GTIN-dedup tie now keeps the twin with the later `EffectiveDateTime` instead of the first-seen one.
...
cargo run download --gtin 10845854038543               # fetch a specific device by UDI-DI primary code (GTIN)
cargo run download --gtin GTIN1 GTIN2 --convert         # multiple GTINs + auto-convert to firstbase JSON
cargo run download --gtin-file gtins.txt --convert      # GTIN list from file (one per line), download + convert
...
cargo run deep-scan srns_sheet.txt --gtin-file gtins_sheet.txt --slices 7   # rolling deep scan (v1.0.108): re-fetch detail + Basic UDI-DI of 1/7 of the worklist per night, compare ALL version parts (UDI, BUDI, market, mfr/AR, cert, package, designer), reconvert + push changed
cargo run deep-scan --gtin-file gtins_sheet.txt --slices 1 --dry-run              # scan the whole GTIN worklist now, report what changed, push nothing
cargo run sync-actors --threads 1 --rate-ms 1200       # defaults; >1 thread risks a /eos 429 lockstep (see CLAUDE.md)
...
- **Production**: `https://webapi-firstbase.gs1.ch` — real data. The GUI has an Environment radio (Test/Production) in the firstbase credentials panel; selecting Production shows a red warning. Production requires separate credentials and a production-valid `Publish To GLN`. From the CLI, the `repush-srn` subcommand pushes to Production when `FIRSTBASE_ENV=Production` is set (anything else, including unset, stays on Test); the NO_LONGER + already-ACCEPTED skip ([#10](https://github.com/zdavatz/eudamed2firstbase/issues/10)) is tracked per environment. Supplier/recipient GLNs come from `config.toml` `[provider]` (`gln` = `7612345000480` EUDAMED Public Importer → token + `DataSource`; `publish_gln` = `7612345000527` Data from EUDAMED → `PublishToGln`), overridable via `FIRSTBASE_PUBLISH_GLN`.
...
| G541 invalid country code 826 (UK/NI) | — | Skip GB from market sales conditions post-Brexit. XI (Northern Ireland) is valid again since the GDSN 2026 release / GS1 UDI Connector Profile Apr 2026 V1.1 and is emitted since v1.0.106 (it was wrongly skipped v1.0.49–v1.0.105, see below) |
...
| [#55](https://github.com/zdavatz/eudamed2firstbase/issues/55) | GS1 Rule | NO_LONGER base unit that is a packaging child: discontinuedDateTime shape | 910.004, 910.003, 097.123 | Open (rule contradiction; CreateMany payload sent to GS1 06.08.2026; child-GTIN reject masking fixed v1.0.100) |
